...
// Manual Debug redacts `api_key` so accidental print/log of an SdkFullConfig
// (including from Python's __repr__, which delegates to Debug) does not leak
// the raw API key.
impl std::fmt::Debug for SdkFullConfig {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SdkFullConfig")
            .field("api_key", &"[redacted]")
            .field("http", &self.http)
            .field("admin", &self.admin)
            .field("streams", &self.streams)
            .field("webhooks", &self.webhooks)
            .field("kvstore", &self.kvstore)
            .finish()
    }
}
...
impl SdkFullConfig {
    ...
    // Hand-rolled to redact `api_key` so to_dict() matches the Debug-based
    // __repr__ behaviour. Users who need the raw key should read .api_key
    // directly.
    fn to_dict<'py>(&self, py: pyo3::Python<'py>) -> pyo3::PyResult<pyo3::Bound<'py, pyo3::PyAny>> {
        ...
        dict.set_item("api_key", "[redacted]")?;
        Ok(obj)
    }
}
...
mod tests {
    ...
    #[test]
    fn sdk_full_config_debug_redacts_api_key() {
        let config = SdkFullConfig::from_api_key("super-secret-key".to_string());
...
mod python_repr_impls {
    use super::*;
    crate::python_repr_dict!(AdminConfig);
    crate::python_repr_dict!(StreamsConfig);
    crate::python_repr_dict!(WebhooksConfig);
    crate::python_repr_dict!(KvStoreConfig);
}
