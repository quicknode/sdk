...
async fn main() {
    ...
    let count = qn.webhooks.get_enabled_count().await.expect("get_enabled_count failed");
    ...
    let webhook =
        qn.webhooks.create_webhook_from_template(&create_params).await.expect("create_webhook_from_template failed");
    let id = webhook.id.clone();
    println!("created: {} | {}", id, webhook.status);
    thread::sleep(Duration::from_secs(1));

    let fetched = qn.webhooks.get_webhook(&id).await.expect("get_webhook failed");
    println!("fetched: {} | {}", fetched.id, fetched.name);

    let update_params =
        UpdateWebhookParams { name: Some("E2E Test Webhook Updated".to_string()), ..Default::default() };
    let updated =
        qn.webhooks.update_webhook(&id, &update_params).await.expect("update_webhook failed");
    println!("updated name: {}", updated.name);
    thread::sleep(Duration::from_secs(1));

    qn.webhooks.pause_webhook(&id).await.expect("pause_webhook failed");
    println!("paused");

    let activate_params = ActivateWebhookParams { start_from: WebhookStartFrom::Latest };
    qn.webhooks.activate_webhook(&id, &activate_params).await.expect("activate_webhook failed");
    println!("activated");

    qn.webhooks.delete_webhook(&id).await.expect("delete_webhook failed");
    println!("deleted: {id}");
    thread::sleep(Duration::from_secs(1));
