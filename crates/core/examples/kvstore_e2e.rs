...
async fn main() {
    ...
    let sets = qn
        .kvstore
        .get_sets(&Default::default())
        .await
        .expect("get_sets failed");
    println!(
        "all sets: {:?}",
        sets.data.iter().map(|e| &e.key).collect::<Vec<_>>()
    );
    ...
    qn.kvstore
        .delete_set("e2e-bulk-key-1")
        .await
        .expect("delete_set failed");
    qn.kvstore
        .delete_set("e2e-bulk-key-2")
        .await
        .expect("delete_set failed");
    ...
    let list = qn
        .kvstore
        .get_list("e2e-list-key", &Default::default())
        .await
        .expect("get_list failed");
    println!("get list items: {:?}", list.data.items);

    let lists = qn
        .kvstore
        .get_lists(&Default::default())
        .await
        .expect("get_lists failed");
    ...
    let contains = qn
        .kvstore
        .list_contains_item("e2e-list-key", "0x123")
        .await
        .expect("list_contains_item failed");
    ...
    qn.kvstore
        .delete_list_item("e2e-list-key", "0x123")
        .await
        .expect("delete_list_item failed");
    println!("deleted list item: 0x123");

    qn.kvstore
        .delete_list("e2e-list-key")
        .await
        .expect("delete_list failed");
